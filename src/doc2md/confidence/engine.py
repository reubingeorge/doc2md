...
from doc2md.types import AgentConfig, ConfidenceLevel, StepResult, VLMResponse
...
class ConfidenceEngine:
    ...
    def aggregate_pipeline(
        ...
        step_scores = {
            name: report.calibrated_score
            for name, report in step_reports.items()
        }
    ...
    def _collect_signals(
        ...
        configured = set(agent_config.confidence.signals) if agent_config.confidence.signals else set()
        ...
        if use_all or "vlm_self_assessment" in configured:
            ...
            signals.append(SignalResult(
                name="vlm_self_assessment", score=score,
                available=available, reasoning=reasoning,
            ))
        ...
        if use_all or "logprobs_analysis" in configured:
            ...
            signals.append(SignalResult(
                name="logprobs_analysis", score=score,
                available=available, reasoning=reasoning,
            ))
        ...
        if use_all or "validation_pass_rate" in configured:
            score, available, reasoning = compute_validation_pass_rate(
                step_result.markdown, agent_config.validation,
            )
            signals.append(SignalResult(
                name="validation_pass_rate", score=score,
                available=available, reasoning=reasoning,
            ))
        ...
        if use_all or "completeness_check" in configured:
            score, available, reasoning = compute_completeness(
                step_result.markdown, agent_config.confidence.expected_fields,
            )
            signals.append(SignalResult(
                name="completeness_check", score=score,
                available=available, reasoning=reasoning,
            ))
        ...
        if use_all or "image_quality" in configured:
            ...
            signals.append(SignalResult(
                name="image_quality", score=score,
                available=available, reasoning=reasoning,
            ))
