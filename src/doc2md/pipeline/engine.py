...
class PipelineEngine:
        ...
        for step_name in execution_order:
            ...
            if step_config.type == StepType.AGENT and step_config.agent:
                ...
                if agent_config:
                    ...
                    signal_data = {
                        s.name: s.score for s in report.signals if s.available
                    }
                    if signal_data:
                        blackboard.write(
                            "confidence_signals", step_name,
                            signal_data, writer=step_name,
                        )
    ...
    def _evaluate_condition(step_config: StepConfig, blackboard: Blackboard) -> bool:
        ...
        try:
            ...
            return bool(eval(step_config.condition, {"bb": blackboard, "__builtins__": {
                "any": any, "all": all, "len": len, "str": str, "int": int, "float": float,
            }}))
        except Exception:
